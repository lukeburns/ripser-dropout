...
# ╔═╡ f73c2b77-f91a-4ea2-837f-f9328b66e021
@bind threshold Slider(ts; default=0.53, show_value=true)

# ╔═╡ a0438aa5-d1e2-483c-ac8c-4b16fdc09053
...
function value_count(arr)
	d = Dict()
	for el in arr
		try d[el] += 1 catch (e) d[el] = 1 end
	end
	d
end

# ╔═╡ 42fc3fc2-75cc-45a9-825f-4daaaa824179
function dropout(data, threshold; n=1)
	death_simplices = []
	inclusions = []
	result = ripserer(data; threshold=threshold)
	for g in result[n]
		if g.death_simplex !== nothing
			push!(death_simplices, g.death_simplex)
			n_simplex = vertices(g.death_simplex)
			points = [data[i] for i in n_simplex]
			append!(inclusions, points)
		end
	end
	unique(inclusions)
end
...
function point_counts(data, ts)
	pts = []
	max_weight = 0
	for ε in ts
		append!(pts, dropout(data, ε))
	end
	sort(value_count(pts), byvalue=true)
end

# ╔═╡ e038c415-22a4-4c0c-8a10-c997d1e25260
...
begin
	data = annulus(200, 1, 1.1)
	noisy_data = [data..., (-0.3, -0.3), (0.4, 0.25)]
end

# ╔═╡ 31f884c0-a26c-4e47-a050-1b417dbdea40
pts = dropout(noisy_data, ε)

# ╔═╡ 27dcbdb0-4d27-46ac-99ae-fc5fb6a79f61
begin
	p = scatter(noisy_data, markerstrokewidth=0, label=nothing)
	for x in pts
		scatter!(x; color=:red, markerstrokewidth=0, label=nothing)
	end
end

# ╔═╡ dbb65a26-eb49-4833-a905-75c976446f49
plot(p, title="Blue Dropped")

# ╔═╡ fbdef875-8e3c-4277-b3e6-d2d7be0906e9
begin
	unfiltered = ripserer(noisy_data, threshold=threshold)
	filtered = ripserer(pts, threshold=threshold)
	dropped = length.(unfiltered) - length.(filtered)
	plot(barcode(unfiltered, title="threshold=$threshold"), barcode(filtered, title="ε=$ε (dropped $(dropped[1]))"))
end

# ╔═╡ f639405e-bb94-4fb1-a2ee-e4a8fd733d85
begin
	counts = point_counts(noisy_data, ts)
	vals = values(counts)
	max_count = maximum(vals)
	min_count = minimum(vals)
	min_count, max_count
end
...
begin
	p2 = scatter()
	for pt ∈ counts
		alpha = (pt[2]/max_count)^2 # opacity isn't super easy to see linearly
		scatter!(pt[1], color=:black, markeralpha=alpha, markerstrokewidth=0, label=nothing, title="alpha = (count/max_count)²")
	end
	p2
end

# ╔═╡ 804b7252-3ec2-42ff-83a9-48f5574fb4c5
unzip(a) = [getindex.(a, i) for i in 1:length(a[1])]

# ╔═╡ 14d4afdf-6825-47f3-b9bc-90d3de13efb4
X = [unzip([keys(counts)...])..., [values(counts)...]] # 😨

# ╔═╡ d6e180e2-49b7-489a-bb77-bceee0281a3f
scatter(X[1], X[2], X[3], markerstrokewidth=0, alpha=0.5, zlabel="Count", zlims=(min_count, max_count)) # 3D weighting
...
git-tree-sha1 = "c7169233feb64e20de4ef96ec83110d6207438a4"
